<?php
$VIMG_VERS = "1.06";		// script version

/* ex:set ts=4 sw=4:
 * VerifyImg PHP script.  This script requires PHP 4 or later.
 * Copyright (c) 2004-2014 Root Software.  All rights reserved.
 *
 ...
 * ~~~~~~~~~~~~~~~
 *
 **Version 1.06: 25-May-2014
 *
 * Fixed bug in calculating the endpoint of vertical lines.
 *
 **Version 1.05: 18-Dec-2009
 *
 * Workaround problems with PHP 5.2.6 and later.
 *
		...
		if ($bCleanText && $ii >= $i_text_left && $ii < $i_text_right)
		{
			...
			else
			{
				...
				$i_start = mt_rand(0,$i_text_top);
                    //
                    // in some cases text_bot can be larger tha height-1
                    // and mt_rand may produce a warning (and return false)
                    // check for this and reverse the parameters
                    //
                if ($i_text_bot > $i_height-1)
                    $i_end = mt_rand($i_height-1,$i_text_bot);
                else
...
imagedestroy($imFinal);
	//
	// Set the verify string in the session, stripping any blank chars.
	//
SetSession("VerifyImgString",str_replace(" ","",$sVerifyString));
?>
